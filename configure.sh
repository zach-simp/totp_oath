...
then
	...
	patch -N -u /etc/pam.d/password-auth patches/00010-password-auth-nofail.patch
else
	# patch without failock in orig
	patch -N -u /etc/pam.d/password-auth patches/00010-password-auth.patch
...
then
	# patch with faillock in orig
	patch -N -u /etc/pam.d/system-auth patches/00010-system-auth-nofail.patch
else
	# patch without failock in orig
	patch -N -u /etc/pam.d/system-auth patches/00010-system-auth.patch
fi
