...
if grep -xq "auth     \[success=1 default=bad\]        pam_oath.so usersfile=/etc/liboath/users.oath window=1" "/etc/pam.d/sshd"
then
	echo "Skipping..."
else
	patch -r - -N -u /etc/pam.d/sshd patches/00010-sshd.patch
fi
